#!/bin/bash
# Azure Pipelines Agent Startup Script
#
# This script configures and runs the Azure Pipelines agent in Docker/Kubernetes.
#
# Required Environment Variables:
#   AZP_URL          - Azure DevOps organization URL (e.g., https://dev.azure.com/myorg)
#   AZP_TOKEN        - Personal Access Token with "Agent Pools" scope (Manage agent pools and agents)
#                      OR
#   AZP_TOKEN_FILE   - Path to file containing the PAT token
#   AZP_POOL         - Agent pool name (default: "Default")
#   AZP_AGENT_NAME   - Agent name (default: hostname)
#   AZP_WORK         - Work directory for agent (default: "_work")
#
...
#   2. Click "+ New Token"
#   3. Name: "azure-devops-agent-operator"
#   4. Select "Custom defined" scope
#   5. Enable: Agent Pools → Manage agent pools and agents (READ & MANAGE)
#   6. Click Create and copy the token immediately
#
# Agent Pool Creation:
#   1. Go to Organization settings → Agent pools
#   2. Click "+ New agent pool"
#   3. Name: Enter exact pool name to use (e.g., "shared-pool")
#   4. Type: Select "Self-hosted"
#   5. Click Create
#
# Kubernetes Secret:
...
if [ -f ./agent/run.sh ]; then
  print_header "1. Agent already present, skipping download..."
else
